...
namespace CoConnect.Infrastructure.Service
{
    public class ServiceBusMiddleware
    {
        ...
        static ServiceBusMiddleware()
        {
            ...
            // Load types from CoConnect.Domain assembly
            var domainAssembly = typeof(CoConnect.Domain.INotificationDispatcher).Assembly;
            ...
            try
            {
                ...
                if (req.Path.StartsWithSegments("/api/command", StringComparison.OrdinalIgnoreCase, out PathString remaining))
                {
                    var modelType = _messageTypes
                        .FirstOrDefault(t => t.FullName.EndsWith(remaining.ToString().Replace("/", ""), StringComparison.OrdinalIgnoreCase));
                    ...
                    if (modelType != null)
                    {
                        ...
                        _logger.LogInformation("Command JSON: {Json}", jsonString);
